...
from PyQt5.QtWidgets import QApplication, QMessageBox, QFileDialog, QMainWindow, QWidget, QMenu, QSlider, QToolTip, QLabel, QVBoxLayout, QLineEdit, QHBoxLayout, QToolButton, QStyle, QStyleOptionSlider, QStackedWidget, QComboBox, QPushButton
...
def run_ocr_and_after(snip_time, ocr, on_error, image, qimg_full, snip_index, rect, audio_bytes, audio_start_time):
    ...
    ocr_result = run_ocr(image,ocr,on_error)
    ...
    # if ocr successful, go search dictionary
        ...
        if word_info['spell'] !=  '':
            ...
            if len(audio_bytes) > 0:
                ...
                word_info['audio_play_time'] = f"{float(play_start_time)},{float(play_end_time)}"
...
def run_ocr(image,ocr,on_error):
    bits = image.bits()
...
def detect_subtitle(points,snip_index,snip_time,audio_bytes,rect,word,ocr,on_error):
    ...
    rect_small = QRect(round(x1 + margin), round(y1 + margin), round(x2 - x1 - margin * 2), round(y2 - y1 - margin * 2))
    ...
    for i in range(snip_index_in_sentences-1,-1,-1):
        last_hog, ocr_budget_left = sentences_one_compare(i,snip_index_in_sentences,snip_index,rect_small,sentences,rect,word,ocr_budget_left,last_hog)
...
def sentences_one_compare(i,snip_index_in_sentences,snip_index,rect_small,sentences,rect,word,ocr_budget_left,last_hog):
    ...
    sentences[i][0] = diff
    if ambiguous_diff_min < diff and diff < ambiguous_diff_max and ocr_budget_left > 0:
        ocr_budget_left -= 1
        ocr_result = run_ocr(screenshot[k][0].copy(rect),ocr,on_error)
        if ocr_result:
            if word == ocr_result[0][0]:
                sentences[i][2] = True
    return this_hog, ocr_budget_left
...
def set_qt_layout():
    ...
    screen_h = mon['height']
    font_size_large = max(20, int(screen_h * 0.020))
    font_size_small = max(10, int(screen_h * 0.008))
    font_size_title = max(11, int(screen_h * 0.008))
    font_size_btn = max(16, int(screen_h * 0.012))
    bar_height = max(32, int(screen_h * 0.037))
    ...
    title_margin = max(8, int(screen_h * 0.012))
    ...
    font_size_settings = max(10, int(screen_h * 0.008))
...
setTimeout(function () {
    // Preload audio + waveform + play time + image bitmap for the back side
        ...
        // Parse audio_play_time field
        var playTime = null;
        var rawPT = "{{audio_play_time}}".trim();
        if (rawPT) {
...
(function () {
    ...
    if (!filename) {
        document.querySelector(".ap-wrap").innerHTML =
            '<div style="color:#c00">No audio filename found in field.</div>';
        return;
    }
...
// Smart fade: duration scales with loudness at start/end
    ...
    (function loadAudio() {
        ...
        function onDecoded(decoded, prerenderedBars, prerenderedPlayTime) {
            ...
            // Priority: localStorage > field audio_play_time > full track
            let initStart = null,
            initEnd = null;
            // 1. Try localStorage
            ...
            if (!pt) {
                const raw = "{{audio_play_time}}".trim();
                if (raw) {
